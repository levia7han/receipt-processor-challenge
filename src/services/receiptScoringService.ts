...
export class ReceiptScoringService {
    public scoreReceipt(receipt: Receipt): number {
        let score = 0
        console.log(receipt)
        score += this.scoreRetailerName(receipt.retailer)
        score += this.scoreTotalRoundAmount(receipt.total)
        score += this.scoreTotalMultiple(receipt.total)
        score += this.scoreItemCount(receipt.items)
        score += this.scorePurchaseDate(receipt.purchaseDate)
        score += this.scoreTimeWindow(receipt.purchaseTime)
        score += this.scoreItemDescriptions(receipt.items)
